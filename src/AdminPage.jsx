...
export default function AdminPage() {
  ...
  const LABELS = {
    pricePerSqm: "Preț/m²", pricePerMeter: "Preț/m", price: "Preț fix",
    pricePerUnit: "Preț/buc", basePrice: "Preț de bază"
  };
  ...
  const renderFields = (productKey, obj, prefix = "") => {
    return Object.entries(obj).map(([key, val]) => {
      if (typeof val === "object" && val !== null) {
        return renderFields(productKey, val, prefix ? `${prefix}.${key}` : key);
      }
      if (PRICE_KEYS.includes(key)) {
        const path = prefix ? `${prefix}.${key}` : key;
        const parentKey = prefix.split(".").pop();
        const parentObj = prefix.split(".").reduce((o, k) => o?.[k], catalog.products[productKey]);
        const label = LABELS[key] || key;
        const name = obj.name || parentKey;
        return (
          <div key={path} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 16px", borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
            <div>
              <div style={{ fontSize: "0.85rem", color: "#f0ede8", fontWeight: 500 }}>{name}</div>
              <div style={{ fontSize: "0.75rem", color: "rgba(240,237,232,0.35)" }}>{label}</div>
            </div>
      ...
      <main style={{ maxWidth: 800, margin: "0 auto", padding: "32px 24px" }}>
        ...
        <div style={{ marginTop: 32, padding: "20px 24px", background: "rgba(255,255,255,0.02)", borderRadius: 16, border: "1px solid rgba(255,255,255,0.07)" }}>
          <p style={{ fontSize: "0.82rem", color: "rgba(240,237,232,0.35)", lineHeight: 1.7, margin: 0 }}>
            <strong style={{ color: "rgba(240,237,232,0.6)" }}>Cum funcționează:</strong> Modifică prețurile de mai sus → apasă <strong style={{ color: "#c8a96e" }}>Export catalog.json</strong> → înlocuiește fișierul <code style={{ color: "#c8a96e" }}>public/catalog.json</code> pe GitHub → Vercel face deploy automat.
          </p>
        </div>
      </main>
    </div>
