...
export default function BalustradeConfiguratorPage() {
  ...
  const calculatePrice = async () => {
    ...
    setQuote({
      ...
      details: {
        glass: productData.glassTypes[glassType].name,
        mounting: productData.mountingTypes[mountingType].name,
      }
    });
              ...
              <div className="grid md:grid-cols-2 gap-6">
                <div>
                  ...
                  <input 
                    ...
                    className="w-full border-2 border-slate-200 p-4 rounded-xl focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none transition-all"
                    placeholder="Ex: 3.5"
                  />
                </div>
                <div>
            ...
            <div className="bg-white rounded-2xl shadow-lg p-8 sticky top-8 border-t-4 border-blue-500">
              ...
              {!showResult ? (
                <div className="text-center py-8">
                  <p className="text-slate-500 mb-6">Introduceți dimensiunile pentru a calcula prețul estimativ</p>
                  <button
                    onClick={calculatePrice}
                    disabled={!dimensions.length || !dimensions.height || calculating}
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100">
      {/* Header */}
      <header className="bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 text-white py-6 px-8 shadow-lg">
        <div className="max-w-6xl mx-auto flex items-center gap-4">
          <Link to="/" className="p-2 rounded-full hover:bg-white/10 transition-all hover:scale-110">
            <ArrowLeft className="w-6 h-6" />
          </Link>
      ...
      <main className="max-w-5xl mx-auto p-8">
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white rounded-2xl shadow-lg p-8">
              ...
              <div className="grid md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  >
                    <option value="">Alege...</option>
                    <option value="0.9">0.9m (balustradă joasă)</option>
                    <option value="1.0">1.0m (standard)</option>
                    <option value="1.1">1.1m</option>
                    <option value="1.2">1.2m (standard exterior)</option>
                    <option value="1.5">1.5m (scări publice)</option>
                  </select>
                </div>
              </div>
            ...
            {/* Tip Sticlă - dinamic din catalog */}
            <div className="bg-white rounded-2xl shadow-lg p-8">
              <div className="flex items-center gap-3 mb-6">
                <div className="p-3 bg-cyan-100 rounded-xl">
                  <GlassWater className="w-6 h-6 text-cyan-600" />
            ...
            {/* Tip Montare - dinamic din catalog */}
            <div className="bg-white rounded-2xl shadow-lg p-8">
              <div className="flex items-center gap-3 mb-6">
                <div className="p-3 bg-purple-100 rounded-xl">
                  <Wrench className="w-6 h-6 text-purple-600" />
                </div>
                <h2 className="text-xl font-bold">Sistem de Prindere</h2>
              </div>
            ...
            <div className="bg-white rounded-2xl shadow-lg p-8">
              ...
              <div className="space-y-4">
                <label className="flex items-center justify-between p-4 border-2 border-slate-200 rounded-xl cursor-pointer hover:border-slate-300 transition-all">
                  <div className="flex items-center gap-3">
                    <input 
                      type="checkbox"
                      checked={includeHandrail}
                    ...
                    />
                    <div>
                      <div className="font-semibold">{productData.options.handrail.name}</div>
                      <div className="text-sm text-slate-500">{productData.options.handrail.description}</div>
                    </div>
                  </div>
                  <span className="text-slate-600 font-medium">{productData.options.handrail.pricePerMeter}€/m</span>
                ...
                <label className="flex items-center justify-between p-4 border-2 border-slate-200 rounded-xl cursor-pointer hover:border-slate-300 transition-all">
                  <div className="flex items-center gap-3">
                    ...
                    />
                    <div>
                      <div className="font-semibold">{productData.options.led.name}</div>
                      <div className="text-sm text-slate-500">{productData.options.led.description}</div>
                    </div>
                  </div>
                  <span className="text-slate-600 font-medium">{productData.options.led.price}€</span>
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-2xl shadow-lg p-8 sticky top-8">
              <h3 className="text-lg font-bold mb-6">Rezumat Ofertă</h3>
              
              {!showResult ? (
                <div className="text-center py-8">
                  <p className="text-slate-500 mb-4">Introduceți dimensiunile pentru a vedea prețul</p>
                  <button
                    onClick={calculatePrice}
                    disabled={!dimensions.length || !dimensions.height || calculating}
                ...
                <div className="space-y-4 animate-fade-in">
                  <div className="space-y-2 text-sm border-b border-slate-200 pb-4">
                    ...
                    <div className="flex justify-between">
                      <span className="text-slate-600">Sticlă ({quote.details.glass}):</span>
                      <span>{quote.glassPrice}€</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-600">Montare ({quote.details.mounting}):</span>
                      <span>{quote.mountingPrice}€</span>
                    </div>*
