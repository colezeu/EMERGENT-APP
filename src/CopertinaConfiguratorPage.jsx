...
const FALLBACK = {
  ...
  basePrice: 300,
  typeCategories: {
    "copertina-tiranti": { name: "Copertină cu Tiranți", pricePerSqm: 380, desc: "Structură susținută cu tiranți inox" },
    "copertina-fara":    { name: "Copertină fără Tiranți", pricePerSqm: 350, desc: "Consolă, fără suport vizibil" },
    "copertina-spider":  { name: "Copertină Spider", pricePerSqm: 490, desc: "Prinderi punctuale, aspect minimal" },
  },
  glassTypes: {
  ...
  options: {
    ...
    mobilier:      { name: "Mobilier Terasă",       price: 1200,       desc: "Set rezistent UV" },
...
function CopertinaPreview({ dims, type, glass, inclLed }) {
  ...
  const roofFill = glass === "solar" ? "rgba(80,160,100,0.12)" : "rgba(180,220,255,0.08)";
  const roofStroke = "rgba(180,220,255,0.4)";
  return (
    <svg width="100%" viewBox={`0 0 ${W} ${H}`}>
      ...
      {type === "copertina-spider" && <>
        <circle cx={x0}    cy={y0}    r={4} fill="rgba(200,169,110,0.7)"/>
        <circle cx={x0+gW} cy={y0}    r={4} fill="rgba(200,169,110,0.7)"/>
        <circle cx={x0}    cy={y0+gD} r={4} fill="rgba(200,169,110,0.7)"/>
        <circle cx={x0+gW} cy={y0+gD} r={4} fill="rgba(200,169,110,0.7)"/>
      </>}
      <text x={x0+gW/2} y={H-6} textAnchor="middle" fill="rgba(200,169,110,0.6)" fontSize="8" fontFamily="DM Sans">
        {dims.width||"—"}m × {dims.depth||"—"}m (plan de sus)
      </text>
    </svg>
  );
}
...
export default function CopertinaConfiguratorPage() {
  ...
  useEffect(() => {
    fetch("/catalog.json")
      ...
      .then(d => {
        ...
        // Filter only copertina types from catalog
        setProduct({
          ...raw,
  ...
  const calculate = async () => {
    ...
    const glP   = area * (p.glassTypes[glass]?.pricePerSqm || 0); // all copertina types use glass
    ...
    setQuote({
      area: area.toFixed(2), typeP: Math.round(typeP), glP: Math.round(glP),
      ledP: Math.round(ledP), dezP: Math.round(dezP), mobP, panP: Math.round(panP),
      subtotal, vat, total
    });
