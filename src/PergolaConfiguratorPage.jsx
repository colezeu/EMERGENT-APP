...
const FALLBACK = { name:"Pergolă & Copertină", basePrice:300, typeCategories:{ "pergola-bioclimatica":{name:"Pergolă Bioclimatică",pricePerSqm:650,desc:"Lamele orientabile, ventilație controlată"}, "pergola-sticla":{name:"Pergolă Acoperiș Sticlă",pricePerSqm:580,desc:"Panouri de sticlă culisante sau fixe"}, "pergola-sandwich":{name:"Pergolă Panou Sandwich",pricePerSqm:420,desc:"Panou termoizolant opac"}, "copertina-tiranti":{name:"Copertină cu Tiranți",pricePerSqm:380,desc:"Structură susținută cu tiranți inox"}, "copertina-fara":{name:"Copertină fără Tiranți",pricePerSqm:350,desc:"Consolă, fără suport vizibil"}, "copertina-spider":{name:"Copertină Spider",pricePerSqm:490,desc:"Prinderi punctuale, aspect minimal"} }, glassTypes:{ clear:{name:"Sticlă Clară",pricePerSqm:0,desc:"Lumină maximă"}, solar:{name:"Control Solar",pricePerSqm:85,desc:"Reduce căldura, g=0.35"}, frosted:{name:"Sablată / Satinată",pricePerSqm:45,desc:"Difuzie lumină"} }, options:{ led:{name:"Iluminare LED",pricePerMeter:55,desc:"Bandă LED în profil"}, dezghetare:{name:"Dezghețare Electrică",pricePerSqm:95,desc:"Rezistențe anti-îngheț"}, mobilier:{name:"Mobilier Pergolă",price:1200,desc:"Set rezistent UV"}, "panouri-lat":{name:"Panouri Laterale",pricePerSqm:280,desc:"Închideri laterale suplimentare"} } };

const isGlassType = t => t.startsWith("pergola-sticla") || t.startsWith("copertina");
  ...
  return (
    <svg width="100%" viewBox={`0 0 ${W} ${H}`}>
      ...
      {isBio&&Array.from({length:slats},(_,i)=>(
        <line key={i} x1={x0+(i/slats)*gW} y1={y0} x2={x0+(i/slats)*gW} y2={y0+gD}
      ...
      {/* tirant lines for copertina-tiranti */}
      ...
      <text x={x0+gW/2} y={H-6} textAnchor="middle" fill="rgba(200,169,110,0.6)" fontSize="8" fontFamily="DM Sans">{dims.width||"—"}m × {dims.depth||"—"}m (plan de sus)</text>
    </svg>
  );
}
...
export default function PergolaConfiguratorPage() {
  ...
  const [type,setType]=useState("pergola-bioclimatica"); const [glass,setGlass]=useState("clear");
  const [inclLed,setInclLed]=useState(false); const [inclDez,setInclDez]=useState(false);
  const [inclMob,setInclMob]=useState(false); const [inclPan,setInclPan]=useState(false);
  ...
  useEffect(()=>{ fetch("/catalog.json").then(r=>r.json()).then(d=>{setProduct(d.products["pergola-copertina"]);setVatRate(d.vatRate);}).catch(()=>setProduct(FALLBACK)); },[]);

  const p=product; const isValid=dims.width&&dims.depth&&parseFloat(dims.width)>0;
  const showGlass=isGlassType(type);
  ...
  return (
    <div style={{minHeight:"100vh",background:"#0f1117",color:"#f0ede8"}}>
      <QuoteModal isOpen={showModal} onClose={()=>setShowModal(false)} quote={quote} productName="Pergolă / Copertină"/>
      <ConfigHeader title="Configurator Pergole & Copertine" quote={quote}/>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:20}}>
          <SectionCard num="01" label="Dimensiuni">
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
              ...
              <NumberInput label="Adâncime / Proiecție (m)" value={dims.depth} onChange={v=>setDims(d=>({...d,depth:v}))} placeholder="Ex: 3.0"/>
            </div>
          </SectionCard>
          <SectionCard num="02" label="Tip Pergolă / Copertină">
